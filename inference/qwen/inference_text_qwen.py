...
"""
Text-output inference script for locally hosted Qwen (via vLLM OpenAI-compatible server).
SpatialIQ tasks 1-10 and main return text (counts/answers).
Saves taskN_pred_text_<slug>.txt and task_main_pred_text_<slug>.txt per view (slug = parent folder, e.g. qwen).
"""
...
def run_spatialiq_benchmark(
    ...
    print(f"SpatialIQ benchmark (text out / Qwen): {label}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Qwen via local vLLM server (text output: SpatialIQ benchmark)."
    )
    parser.add_argument(
        "folder",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="T",
        help="Sampling temperature (default: 0 from QWEN_TEMPERATURE env or 0). Lower = less random, usually shorter.",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
        ...
        dest="max_tokens",
        metavar="N",
        help="Max new tokens per reply (default: 128 or QWEN_MAX_TOKENS). Caps long reasoning.",
    )
    parser.add_argument(
        "--top-p",
        type=float,
        ...
        dest="top_p",
        metavar="P",
        help="Nucleus sampling; omit to skip (default: unset, or QWEN_TOP_P if set).",
    )
    parser.add_argument(
        "--slug",
        type=str,
        ...
        metavar="SLUG",
        help=(
            "Override prediction filename slug (taskN_pred_text_<slug>.txt). "
