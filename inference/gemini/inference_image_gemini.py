...
"""
Image-output inference script for NVIDIA Inference API (Gemini 3 Pro).
For text-output SpatialIQ tasks use inference_text_gemini.py instead.
Requires a folder path: run SpatialIQ benchmark (image out) on that sample folder or discover all samples under it. Saves task1_pred_image.png .. task10_pred_image.png and task_main_pred_image.png per view.
"""
...
def run_spatialiq_benchmark(
    ...
    Tasks 1-9 and Main: one image per view → save taskN_pred_image.png, task_main_pred_image.png per view.
    Task 10: (view_0, view_N) for each N>=1 → save task10_pred_image.png in view_N's directory.
    """
    if task_prompts is None or main_task_prompt is None:
        raise ValueError("task_prompts and main_task_prompt must be provided.")
    ...
    for v_idx, img_path, out_dir in zip(view_indices, image_paths, output_dirs):
        ...
        if run_main:
            main_path = os.path.join(out_dir, f"task_main_pred_image_{MODEL_SLUG}.png")
            ...
            if not override and os.path.isfile(main_path):
                ...
                try:
                    img_bytes = call_api_image_and_text_to_image(img_path, main_task_prompt, api_key, debug=debug)
    ...
    # Cross-view tasks (task10, task11): two reference images → image output
            ...
            if not override and os.path.isfile(out_path):
                ...
                try:
                    img_bytes = call_api_two_images_and_text_to_image(
                        img_path, paired_path, task_prompts.get(cv_task, ""), api_key, debug=debug
                    )
                    elapsed = time.monotonic() - t0
                    if img_bytes:
                        with open(out_path, "wb") as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Gemini 3 Pro via NVIDIA Inference API (image output: SpatialIQ benchmark)."
    )
    parser.add_argument(
        "folder",
    ...
    for i, (label, views) in enumerate(samples, 1):
        ...
        run_spatialiq_benchmark(views, api_key, label=label, debug=args.debug, tasks=tasks_set, override=args.override, task_prompts=task_prompts, main_task_prompt=main_task_prompt)
        if len(samples) > 1:
            print()
