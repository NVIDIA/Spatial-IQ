...
"""
FRQ (free-response, text-output) inference for Nemotron via NVIDIA Inference API.

Model: nemotron-3-nano-omni-30b-a3b-reasoning (endpoint the configured inference endpoint).
Sends a single view image plus the task prompt as OpenAI-compatible multimodal
content. Reasoning is DISABLED via `chat_template_kwargs={"enable_thinking": false}`
so the model returns a short direct answer (otherwise the chain-of-thought blows
through the 32k context window before any `content` is emitted).

Cross-view tasks (task10, task11) are skipped — this model accepts at most
ONE image per prompt and so cannot be given two views simultaneously.
"""
...
def _extract_text(content) -> str:
    """Extract plain text from an API content field that may be a string or a list of content blocks."""
...
def call_api_image_and_text(image_path: str, prompt: str, api_key: str) -> str:
    """Image + text → text: send one image and the prompt with reasoning disabled,
...
def call_api_two_images_and_text(
    image_path_1: str, image_path_2: str, prompt: str, api_key: str
) -> str:
    """Not supported: this Nemotron variant accepts at most one image per prompt."""
    raise RuntimeError(
        "nemotron-3-nano-omni-30b-a3b-reasoning accepts at most 1 image per prompt; "
        "cross-view tasks (task10/task11) cannot be run with this model."
    )
...
def parse_tasks_arg(tasks_str: str) -> set:
    """Parse --tasks string (e.g. '1,2,10,main') into set of task names. Default: all tasks."""
...
def run_spatialiq_benchmark(
    ...
    """Run selected SpatialIQ text-output tasks on the given views.
    Tasks 1-9: one image per view → save taskN_pred_text.txt. Task 10: (view_0, view_N) → task10_pred_text.txt. Main: task_main_pred_text.txt.
    """
    ...
    print(f"SpatialIQ benchmark (text out): {label}")
            ...
            if not override and os.path.isfile(out_path):
                ...
                try:
                    answer = call_api_image_and_text(
                        img_path, task_prompts[task_name], api_key
                    )
    ...
    # This Nemotron variant rejects requests with >1 image, so cross-view tasks
    # are skipped here even when requested.
    cv_requested = [t for t in ("task10", "task11") if t in run_tasks]
    if cv_requested:
        print(f"Skipping cross-view tasks {cv_requested}: "
              f"{MODEL} accepts at most 1 image per prompt.\n")
    for cv_task, step in []:
            ...
            if not override and os.path.isfile(out_path):
                ...
                try:
                    answer = call_api_two_images_and_text(
                        img_path, paired_path, prompt_cv, api_key
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Nemotron-3-Nano-Omni-30B-A3B (reasoning) via NVIDIA Inference API "
                    "(FRQ image+text output for SpatialIQ; cross-view tasks skipped: 1-image limit)."
    )
    parser.add_argument(
        "folder",
    ...
    parser.add_argument(
        "--reverse",
        action="store_true",
        help="Process samples in reverse order (bottom-up). Useful for splitting work across two keys.",
    )
    parser.add_argument(
        "--start",
        type=int,
        default=1,
        metavar="N",
        help="1-indexed sample to start from (default: 1). Applied after --reverse.",
    )
    parser.add_argument(
        "--prompts",
        type=str,
    ...
    if args.start > 1:
        if args.start > total:
            raise SystemExit(f"--start {args.start} exceeds {total} sample(s) available.")
        samples = samples[args.start - 1:]
    qualifiers = []
    if args.reverse:
        qualifiers.append("reversed")
    if args.start > 1:
        qualifiers.append(f"starting at sample {args.start}")
    if qualifiers:
        print(f"Processing {len(samples)} of {total} samples ({', '.join(qualifiers)}).\n")
    for i, (label, views) in enumerate(samples, 1):
        if len(samples) > 1:
            print(f"========== Sample {i}/{len(samples)}: {label} ==========")
        run_spatialiq_benchmark(views, api_key, label=label, tasks=tasks_set, override=args.override, task_prompts=task_prompts, main_task_prompt=main_task_prompt)
        if len(samples) > 1:
            print()
