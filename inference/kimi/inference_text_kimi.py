...
"""
Text-output inference script for NVIDIA Inference API (Kimi K2.5).
SpatialIQ tasks 1–10 and main return text (counts/answers). For image-output tasks use inference_image_gemini.py.
Requires a folder path: run SpatialIQ benchmark (text out) on that sample folder or discover all samples under it. Saves task1_pred_text.txt .. task10_pred_text.txt and task_main_pred_text.txt per view.
"""
...
def _stream_response(resp: requests.Response) -> str:
    """Consume an SSE stream and return the assembled content (ignoring reasoning tokens)."""
    parts = []
...
def _call_api(content: list, api_key: str) -> str:
    """Send a chat completion request with streaming and return the content."""
    resp = requests.post(
        API_BASE,
        headers={
...
def call_api_two_images_and_text(
    ...
    """Two images + text → text: send first image, second image, and prompt; return the assistant message."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Kimi K2.5 via NVIDIA Inference API (text output: addition if no folder, SpatialIQ if folder given)."
    )
    parser.add_argument(
        "folder",
