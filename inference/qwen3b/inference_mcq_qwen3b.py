...
"""
MCQ inference script for locally hosted Qwen2.5-VL-3B (via vLLM OpenAI-compatible server).
SpatialIQ tasks 1-11 and main: send reference image(s) + 5 MCQ choice images,
expect a single letter (A-E) answer.
Saves taskN_pred_mcq_<slug>.txt and task_main_pred_mcq_<slug>.txt per view (slug = parent folder, e.g. qwen3b).
"""
...
def run_spatialiq_mcq_benchmark(
    ...
    print(f"SpatialIQ benchmark (MCQ / Qwen2.5-3B): {label}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Qwen2.5-VL-3B via local vLLM server (MCQ: send reference + choice images, get A-E answer)."
    )
    ...
    parser.add_argument("--prompts", type=str,
        default=os.path.join(REPO_ROOT, "inference", "prompt_mcq_Type1.json"), metavar="FILE",
