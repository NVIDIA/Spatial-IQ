...
"""
Text-output inference script for NVIDIA Inference API (Claude Opus 4.6 via Bedrock).
SpatialIQ tasks 1-10 and main return text (counts/answers).
Requires a folder path: run SpatialIQ benchmark (text out) on that sample folder or discover all samples under it.
Saves task1_pred_text.txt .. task10_pred_text.txt and task_main_pred_text.txt per view.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Claude Opus 4.6 via NVIDIA Inference API (text output: SpatialIQ benchmark)."
    )
    parser.add_argument(
        "folder",
