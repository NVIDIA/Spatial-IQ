...
"""
Text-output inference script for locally hosted GLM (via vLLM OpenAI-compatible server).
SpatialIQ tasks 1-10 and main return text (counts/answers).
Saves taskN_pred_text_<slug>.txt and task_main_pred_text_<slug>.txt per view (slug = parent folder, e.g. glm).
"""
...
def _strip_glm_special_tokens(text: str) -> str:
    """Remove GLM reasoning / box tokens and return the clean payload."""
...
def call_api_image_and_text(
    ...
    return _strip_glm_special_tokens(text)
...
def _get_paired_view_path(img_path: str, step: int) -> Optional[str]:
    ...
    Assumes 1-based view indices (view_01..view_04) with NUM_VIEWS total views.
    Returns None if the view index cannot be parsed from the directory structure.
    """
...
def load_prompts(prompts_file: Optional[str]) -> Tuple[dict, str]:
    ...
    The JSON should have a "task_prompts" key containing task1..task11 and main.
...
def run_spatialiq_benchmark(
    ...
    """Run selected SpatialIQ text-output tasks + Main Task for the given ordered views.
    Tasks 1-9: one image per view -> save taskN_pred_text_<slug>.txt.
    Task 10: (view_0, view_N) -> task10_pred_text_<slug>.txt.
    Main: task_main_pred_text_<slug>.txt.
    """
    ...
    print(f"SpatialIQ benchmark (text out / GLM): {label}")
    print(f"  Views: {view_indices} ({len(views)} total)")
    print(f"  Tasks: {sorted(run_tasks)}")
    print(f"  Model: {model}\n")

    # Single-view tasks (1-9) and Main per view
...
def main():
    parser = argparse.ArgumentParser(
        description="Run GLM via local vLLM server (text output: SpatialIQ benchmark)."
    )
    parser.add_argument(
        "folder",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="NAME",
        help=f"Served model name (default: {DEFAULT_MODEL} or GLM_MODEL env var).",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=None,
        metavar="T",
        help="Sampling temperature (default: 0 from GLM_TEMPERATURE env or 0).",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
        ...
        dest="max_tokens",
        metavar="N",
        help="Max new tokens per reply (default: 128 or GLM_MAX_TOKENS).",
    )
    parser.add_argument(
        "--top-p",
        type=float,
        ...
        dest="top_p",
        metavar="P",
        help="Nucleus sampling; omit to skip (default: unset, or GLM_TOP_P if set).",
    )
    args = parser.parse_args()

    base_url = (args.base_url or os.environ.get("VLLM_BASE_URL") or DEFAULT_BASE_URL).strip()
    model = (args.model or os.environ.get("GLM_MODEL") or DEFAULT_MODEL).strip()

    temperature = (
        args.temperature
        if args.temperature is not None
        else float(os.environ.get("GLM_TEMPERATURE", "0"))
    )
    max_tokens = (
        args.max_tokens
        if args.max_tokens is not None
        else int(os.environ.get("GLM_MAX_TOKENS", "128"))
    )
    top_p = args.top_p
    if top_p is None and os.environ.get("GLM_TOP_P", "").strip() != "":
        top_p = float(os.environ["GLM_TOP_P"])
