...
"""
MCQ inference script for locally hosted GLM (via vLLM OpenAI-compatible server).
SpatialIQ tasks 1-10 and main: send reference image(s) + 5 MCQ choice images,
expect a single letter (A-E) answer.
Saves taskN_pred_mcq_<slug>.txt and task_main_pred_mcq_<slug>.txt per view (slug = parent folder, e.g. glm).
"""
...
def run_spatialiq_mcq_benchmark(
    ...
    print(f"SpatialIQ benchmark (MCQ / GLM): {label}")
        ...
        for v_idx, img_path, data_dir, out_dir in zip(view_indices, image_paths, data_dirs, output_dirs):
            ...
            if not override and os.path.isfile(out_path):
                ...
                if missing:
                    ...
                    t0 = time.monotonic()
                    try:
                        answer = call_api_multi_image(all_images, task_prompts.get(cv_task, TASK_PROMPTS.get(cv_task, "")), client, model, sampling=sampling)
                        elapsed = time.monotonic() - t0
                        if answer.strip():
                            with open(out_path, "w") as f:
                                f.write(answer.strip())
...
def main():
    parser = argparse.ArgumentParser(
        description="Run GLM via local vLLM server (MCQ: send reference + choice images, get A-E answer)."
    )
    parser.add_argument(
        "folder",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="T",
        help="Sampling temperature (default: GLM_TEMPERATURE env or 0).",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
        ...
        dest="max_tokens",
        metavar="N",
        help="Max new tokens (default: GLM_MAX_TOKENS env or 32 for MCQ).",
    )
    parser.add_argument(
        "--top-p",
        type=float,
        ...
        dest="top_p",
        metavar="P",
        help="Optional nucleus sampling (default: GLM_TOP_P env if set).",
    )
    ...
    max_tokens = (
        ...
        else int(os.environ.get("GLM_MAX_TOKENS", "32"))
    )
    top_p = args.top_p
